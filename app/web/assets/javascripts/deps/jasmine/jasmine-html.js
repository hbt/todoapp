define(['jasmine', 'deps/jasmine/jasmine-flowcharts'], function(jasmine, JF) {
    var jbak = jasmine
    ...
    jasmine.HtmlReporterHelpers.findParentSuite = function(spec) {
    ...
    jasmine.HtmlReporterHelpers.appendGraphiz = function(spec) {
        var specIndex = null

        for(var index in this.views.specs) {
            var vspec = this.views.specs[index]
            if(vspec.spec.id === spec.id) {
                specIndex = parseInt(index)
                break
            }
        }

       var currentParentSuite = this.findParentSuite(spec)
       var nextSuite = null

        if(specIndex > 1) {
           var prevSpec = this.views.specs[specIndex-1].spec
           var prevParentSuite = this.findParentSuite(prevSpec)

           // new parent suite found
           if(currentParentSuite.id !== prevParentSuite.id) {
              nextSuite = prevParentSuite
           }

            // last index?
            if(specIndex === _.keys(this.views.specs).length-1) {
                 nextSuite = currentParentSuite
            }
        }
        ...
        if(nextSuite) {
            ...
            // find the json spec
            var nextJSON = null
            _.each(jasmine.jspecs, function(jspec) {
                var file = "file: " + jspec['_summary']._file
                if(file === nextSuite.description) {
                    nextJSON = jspec;
                }
            })
            ...
            if(nextJSON) {
                ...
                _.each(els, function(el) {
                    if(el.innerHTML === nextSuite.description) {
                        JF.Graphiz.create(nextJSON, {
                            suite: nextSuite,
                            el: el.parentElement
                        })
                    }
