...
--------------------------------------*/
    ...
    getScrollElem = function()
    {
        var scrollElem = $window,
            activePage, touchOverflow = $.support.touchOverflow && $.mobile.touchOverflowEnabled;
	...
	*/
    ...
    {
        return asParsedObject ? $.extend(
        {}, documentBase) : documentBase.href;
    };
    ...
    $.mobile.loadPage = function(url, options)
    {
        ...
        var deferred = $.Deferred(),
            ...
            // the caller.
            settings = $.extend(
            {}, $.mobile.loadPage.defaults, options),
            ...
            $.ajax(
            {
                ...
                error: function(xhr, textStatus, errorThrown)
                {
                    ...
                    if (settings.showLoadMsg)
                    {
                        ...
                        //show error message
                        $("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>" + $.mobile.pageLoadErrorMessage + "</h1></div>").css(
                        {
                            "display": "block",
                            "opacity": 0.96,
                            "top": $window.scrollTop() + 100
                        }).appendTo(settings.pageContainer).delay(800).fadeOut(400, function()
                        {
                            $(this).remove();
                        });
                    }
...
(function($, window, undefined)
{
    ...
    $.widget("mobile.dialog", $.mobile.widget, {
        ...
        _create: function()
        {
            ...
            // Set aria role
            $el.attr("role", "dialog").addClass("ui-dialog").find(":jqmData(role='header')").addClass("ui-corner-top ui-overlay-shadow").prepend(headerCloseButton).end().find(":jqmData(role='content'),:jqmData(role='footer')").addClass("ui-overlay-shadow").last().addClass("ui-corner-bottom");
    ...
    $(":jqmData(role='page'), :jqmData(role='dialog')").live("pagecreate", function(e)
    {
        ...
        $(":jqmData(role='header'), :jqmData(role='footer'), :jqmData(role='content')", this).each(function()
        {
            ...
            if (role === "header" || role === "footer")
            {
                ...
                if (o.addBackBtn && role === "header" && $(".ui-page").length > 1 && $this.jqmData("url") !== $.mobile.path.stripHash(location.hash) && !leftbtn)
                {
...
(function($, undefined)
{
    ...
    $.widget("mobile.collapsible", $.mobile.widget, {
        ...
        _create: function()
        {
            ...
            if (!collapsibleSet.length)
            {
                collapsibleHeading.find("a").first().add(collapsibleHeading.find(".ui-btn-inner")).addClass("ui-corner-top ui-corner-bottom");
            }
            else
            {
                // If we are in a collapsible set
                ...
                if (!collapsibleSet.jqmData("collapsiblebound"))
                {
                    ...
                    collapsibleSet.jqmData("collapsiblebound", true).bind("expand", function(event)
                    {
                ...
                {
                    collapsibleHeading.find("a").first().add(collapsibleHeading.find(".ui-btn-inner")).addClass("ui-corner-bottom");
                }
            }
    ...
    $.widget("mobile.listview", $.mobile.widget, {
        ...
        refresh: function(create)
        {
            ...
            var o = this.options,
                ...
                itemClassDict = {},
                item, itemClass, itemTheme, a, last, splittheme, countParent, icon, imgParents, img;
    ...
    $.widget("mobile.slider", $.mobile.widget, {
        ...
        _create: function()
        {
            ...
            var self = this,
                ...
                handle = $("<a href='#' class='ui-slider-handle'></a>").appendTo(slider).buttonMarkup(
                {
                    corners: true,
                    theme: theme,
                    shadow: true
            ...
            if (cType == "select")
            {
                ...
                control.find("option").each(function(i)
                {
                    ...
                    $("<span class='ui-slider-label ui-slider-label-" + side + theme + " ui-btn-corner-" + corners + "' role='img'>" + $(this).getEncodedText() + "</span>").prependTo(handle);
                });
    ...
    $.widget("mobile.textinput", $.mobile.widget, {
        ...
        _create: function()
        {
            ...
            if (input.is("[type='search'],:jqmData(type='search')"))
            {
                ...
                clearbtn = $("<a href='#' class='ui-input-clear' title='clear text'>clear text</a>").tap(function(event)
                {
                    input.val("").focus();
    ...
    var extendSelect = function(widget)
        {
            ...
            var select = widget.select,
                ...
                menuPage = $("<div data-" + $.mobile.ns + "role='dialog' data-" + $.mobile.ns + "theme='" + widget.options.theme + "' data-" + $.mobile.ns + "overlay-theme='" + widget.options.overlayTheme + "'>" + "<div data-" + $.mobile.ns + "role='header'>" + "<div class='ui-title'>" + label.getEncodedText() + "</div>" + "</div>" + "<div data-" + $.mobile.ns + "role='content'></div>" + "</div>").appendTo($.mobile.pageContainer).page(),
            ...
            $.extend(widget, {
                ...
                build: function()
                {
                    ...
                    self.list.attr("role", "listbox").delegate(".ui-li>a", "focusin", function()
                    {
                ...
                open: function()
                {
                    ...
                    scrollTop = tOverflow ? tScrollElem.scrollTop() : $(window).scrollTop(), btnOffset = self.button.offset().top, screenHeight = window.innerHeight, screenWidth = window.innerWidth;
...
(function($, undefined)
{
    ...
    $(document).bind("pagecreate create", function(e)
    {
        ...
        $(e.target).find("a").not(".ui-btn, .ui-link-inherit, :jqmData(role='none'), :jqmData(role='nojs')").addClass("ui-link");
...
(function($, undefined)
{
    ...
    $.mobile.fixedToolbars = (function()
    {
        ...
        var stickyFooter, delayTimer, currentstate = "inline",
            autoHideMode = false,
            showDelay = 100,
        ...
        $(function()
        {
            ...
            (($document.scrollTop() === 0) ? $window : $document).bind("scrollstart", function(event)
            {
                ...
                $target.each(function()
                {
                    ...
                    if ($fixies.length)
                    {
                        ...
                        if (fullScreen)
                        {
                            ...
                            {
                                $fixies.removeClass("ui-native-bars-hidden").toggleClass("in out").animationComplete(function()
                                {
    ...
    $.extend($.mobile, {
        ...
        showPageLoadingMsg: function()
        {
            if ($.mobile.loadingMessage)
            {
                ...
                $loader.find("h1").text($.mobile.loadingMessage).end().appendTo($.mobile.pageContainer)
